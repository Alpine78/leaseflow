...
invoke-local-scan-due-lease-reminders: check-local-env
	cd $(BACKEND_DIR) && set -a && . ./$(LOCAL_ENV_FILE) && set +a && $(PYTHON) scripts/invoke_local.py scan-due-lease-reminders --tenant-id "$(LOCAL_TENANT_ID)"
